...
void ezHeader::show(String t /* = "" */) {
	...
	M5.Display.fillRect(0, 0, TFT_W, ez.theme->header_height, ez.theme->header_bgcolor);	// Clear header area
	for (uint8_t n = 0; n < _widgets.size(); n++) {
...
void ezHeader::clear(bool wipe /* = true */) {
	if (wipe) M5.Display.fillRect(0, 0, TFT_W, ez.theme->header_height, ez.theme->background);
	_shown = false;
	ez.canvas.top(0);
}
...
void ezHeader::_drawTitle(uint16_t x, uint16_t w) {
	M5.Display.fillRect(x, 0, w, ez.theme->header_height, ez.theme->header_bgcolor);
	M5.Display.setTextDatum(TL_DATUM);
	M5.Display.setTextColor(ez.theme->header_fgcolor);
	ez.setFont(ez.theme->header_font);
	M5.Display.drawString(ez.clipString(_title, w - ez.theme->header_hmargin), x + ez.theme->header_hmargin, ez.theme->header_tmargin);
}	
...
void ezCanvas::clear() {
	M5.Display.fillRect(left(), top(), width(), height(), ez.screen.background());
	_x = _lmargin;
	_y = 0;
	_printed.clear();
}
...
uint16_t ezCanvas::loop() {
	if (_next_scroll && millis() >= _next_scroll) {
		...
		M5.Display.fillRect(left(), top(), width(), height(), ez.screen.background());
		// M5.Display.fillRect(0, 0, 320, 240, ez.screen.background());
		for (uint16_t n = 0; n < _printed.size(); n++) {
			if (_printed[n].y >= _top) {
				if (_printed[n].font != _font) ez.setFont(_printed[n].font);
				if (_printed[n].color != _color) M5.Display.setTextColor(_printed[n].color);
				M5.Display.drawString(_printed[n].text, _printed[n].x, _printed[n].y);
			}
		}
		_y -= scroll_by;
		_font = hold_font;
...
void ezCanvas::_print(String text) {
 	ez.setFont(_font);
 	M5.Display.setTextDatum(TL_DATUM);
	M5.Display.setTextColor(_color, ez.theme->background);
 	uint8_t h = ez.fontHeight();
 	if (_y + h > _bottom) {
 		if (!_scroll) return;
 	...
 	if (_x + M5.Display.textWidth(text) <= _right) {
		if (text != "") _putString(text);
	} else {
		for (uint16_t n = 0; n < text.length(); n++) {
			if (_x + M5.Display.textWidth(text.substring(0, n + 1)) > _right) {
				if (n) {
...
void ezCanvas::_putString(String text) {
	...
	if (_y + h > _bottom) {
		_x += M5.Display.textWidth(text);
	} else{
		_x += M5.Display.drawString(text, _x, _y);
	}
}
...
void ezButtons::clear(bool wipe /* = true */) {
	if (wipe && (_lower_button_row  || _upper_button_row)) {
		M5.Display.fillRect(0, ez.canvas.bottom() + 1, TFT_H - ez.canvas.bottom() - 1, TFT_W, ez.screen.background());
	}
...
void ezButtons::_drawButtons(String btn_a_s, String btn_a_l, String btn_b_s, String btn_b_l, String btn_c_s, String btn_c_l, String btn_ab, String btn_bc, String btn_ac) {
	...
	if (btn_a_s != "" || btn_a_l != "" || btn_b_s != "" || btn_b_l != "" || btn_c_s != "" || btn_c_l != "") {
		if (!_lower_button_row) {
			...
			M5.Display.fillRect(0, TFT_H - ez.theme->button_height - ez.theme->button_gap, TFT_W, ez.theme->button_height + ez.theme->button_gap, ez.screen.background());
		}
	...
	if (btn_ab != "" || btn_bc != "" || btn_ac != "") {
		if (!_upper_button_row) {
			...
			M5.Display.fillRect(0, TFT_H - 2 * (ez.theme->button_height + ez.theme->button_gap), TFT_W, ez.theme->button_height + ez.theme->button_gap, ez.screen.background());
		}
		...
		if (_btn_ac != btn_ac) {
			...
			M5.Display.fillRect(0, TFT_H - 2 * ez.theme->button_height - ez.theme->button_gap, btnwidth / 4, ez.theme->button_height, ez.theme->button_bgcolor_t);
			M5.Display.fillRect(TFT_W - (btnwidth / 4), TFT_H - 2 * ez.theme->button_height - ez.theme->button_gap, btnwidth / 4, ez.theme->button_height, ez.theme->button_bgcolor_t);
...
void ezButtons::_drawButton(int16_t row, String text_s, String text_l, int16_t x, int16_t w) {
	...
	if (text_s != "" || text_l != "") {
		...
		M5.Display.fillRoundRect(x, y, w, ez.theme->button_height, ez.theme->button_radius, bg_color);
		if (text_l != "") {
		...
		M5.Display.fillRect(x, y, w, ez.theme->button_height, ez.screen.background());
	}
}
...
void ezButtons::_drawButtonString(String text, int16_t x, int16_t y, uint16_t color, int16_t datum) {
	...
	if (text == "up" || text == "down" || text == "left" || text == "right") {
		y+=2;
		int16_t w = M5.Display.textWidth("A") * 1.2;
		int16_t h = ez.fontHeight() * 0.6;
		if (datum == TR_DATUM) x = x - w;
		if (datum == TC_DATUM) x = x - w/2; 
		if (text == "up") M5.Display.fillTriangle(x, y + h, x + w, y + h, x + w/2 ,y , color);
		if (text == "down") M5.Display.fillTriangle(x, y, x + w, y, x + w/2 ,y + h, color);
		if (text == "right") M5.Display.fillTriangle(x, y, x, y + h, x + w, y + h/2, color);
		if (text == "left") M5.Display.fillTriangle(x + w, y, x + w, y + h, x, y + h/2, color);		
	} else {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void ezClock::restart() {
		...
		if (_on) {
			...
			uint8_t width = length * M5.Display.textWidth("5") + ez.theme->header_hmargin * 2;
	...
	void ezClock::draw(uint16_t x, uint16_t w) {
		...
		M5.Display.setTextColor(ez.theme->header_fgcolor);
		M5.Display.setTextDatum(TL_DATUM);
		M5.Display.drawString(tz.dateTime(_datetime), x + ez.theme->header_hmargin, ez.theme->header_tmargin + 2);
	}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void ezWifi::_drawWidget(uint16_t x, uint16_t w) {
		...
		for (uint8_t n = 0; n < max_bars; n++) {
			...
			M5.Display.fillRect(left_offset + n * (ez.theme->signal_bar_width + ez.theme->signal_bar_gap), top + max_len - this_len, ez.theme->signal_bar_width, this_len, (n + 1 <= bars ? ez.theme->header_fgcolor : ez.theme->header_bgcolor) );
		}
	}
	...
	bool ezWifi::_connection(ezMenu* callingMenu) {
		...
		void ezWifi::_WPShelper(arduino_event_id_t event, arduino_event_info_t info) {
			_WPS_event = event;
			_WPS_new_event = true;
			// WPS PIN event handling - event type may have changed in newer ESP32 framework
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void ezBattery::_drawWidget(uint16_t x, uint16_t w) {
		...
		M5.Display.fillRoundRect(left_offset, top, ez.theme->battery_bar_width, height, ez.theme->battery_bar_gap, ez.theme->header_bgcolor);
		M5.Display.drawRoundRect(left_offset, top, ez.theme->battery_bar_width, height, ez.theme->battery_bar_gap, ez.theme->header_fgcolor);
		...
		for (uint8_t n = 0; n < (_numChargingBars != BATTERY_CHARGING_OFF ? _numChargingBars : currentBatteryLevel); n++) {
			M5.Display.fillRect(left_offset + n * (bar_width + ez.theme->battery_bar_gap), top + ez.theme->battery_bar_gap, 
...
String M5ez::msgBox(String header, String msg, String buttons /* = "OK" */, const bool blocking /* = true */, const GFXfont* font /* = NULL */, uint16_t color /* = NO_COLOR */) {
	...
	for (int8_t n = 0; n < lines.size(); n++) {
		...
		M5.Display.drawString(lines[n].line, TFT_W / 2, y);
	}
	if (buttons != "" && blocking) {
...
void M5ez::_drawTextInputLockString(String text) {
	...
	M5.Display.fillRect(0, _text_cursor_y + ez.theme->input_vmargin + 10 + text_h, TFT_W, text_h, ez.screen.background());
	M5.Display.drawString(text, TFT_W - ez.theme->input_hmargin - 10, _text_cursor_y + ez.theme->input_vmargin + text_h + 10);
}
...
void M5ez::_drawTextInputBox(String text) {
	...
	_text_cursor_w = M5.Display.textWidth("A");
	M5.Display.fillRoundRect(ez.theme->input_hmargin, ez.canvas.top() + ez.theme->input_top, box_w, text_h + ez.theme->input_vmargin * 2, 8, ez.theme->input_bgcolor);
	_text_cursor_y = ez.canvas.top() + ez.theme->input_top + ez.theme->input_vmargin;
	M5.Display.setTextColor(ez.theme->input_fgcolor);
	String disp_text = text;
	// chop off characters from the beginning of displayed string until it fits
	while (true) {
		text_w = M5.Display.textWidth(disp_text);
	...
	M5.Display.drawString(disp_text, TFT_W / 2 - _text_cursor_w / 2, ez.canvas.top() + ez.theme->input_top + ez.theme->input_vmargin);
...
void M5ez::_textCursor(bool state) {
	if (state) M5.Display.fillRect(_text_cursor_x, _text_cursor_y, _text_cursor_w, _text_cursor_h, ez.theme->input_fgcolor);
	if (!state) M5.Display.fillRect(_text_cursor_x, _text_cursor_y, _text_cursor_w, _text_cursor_h, ez.theme->input_bgcolor);
...
String M5ez::textBox(String header /*= ""*/, String text /*= "" */, bool readonly /*= false*/, String buttons /*= "up#Done#down"*/, const GFXfont* font /* = NULL */, uint16_t color /* = NO_COLOR */) {
	...
	bool redraw = true;
	ez.setFont(font);
	uint8_t cursor_width = M5.Display.textWidth("|");
	uint8_t cursor_height = per_line_h * 0.8;
	int16_t cursor_x = 0;
	int16_t cursor_y = 0;
	while (true) {
		if (redraw) {
			if (!readonly && cursor_x && cursor_y) M5.Display.fillRect(cursor_x, cursor_y, cursor_width, cursor_height, ez.screen.background());		//Remove current cursor
			cursor_x = cursor_y = 0;
			tmp_buttons = buttons;
			...
			ez.setFont(font);
			M5.Display.setTextColor(color, ez.screen.background());
			M5.Display.setTextDatum(TL_DATUM);
			uint16_t x, y;
			int16_t sol, eol;
			String this_line;
			if (lines.size() > 0) {
				for (int8_t n = offset; n < offset + lines_per_screen; n++) {
					...
					if (!readonly && sol != -1 && cursor_pos >= sol && cursor_pos <= eol && n < lines.size()) { 		// if cursor is on current line
						x += M5.Display.drawString(this_line.substring(0, cursor_pos - sol), x, y);
						cursor_x = x;
						cursor_y = y;
						x += cursor_width;
						x += M5.Display.drawString(this_line.substring(cursor_pos - sol), x, y);
					} else {
						x += M5.Display.drawString(this_line, x, y);
					}
					M5.Display.fillRect(x, y, ez.canvas.width() - x, per_line_h, ez.screen.background());
				}
				redraw = false;
			}
		}
		if (!readonly && cursor_x && cursor_y && millis() - cursor_time > ez.theme->input_cursor_blink) {
			cursor_time = millis();
			if (cursor_state) {
				M5.Display.fillRect(cursor_x, cursor_y, cursor_width, cursor_height, ez.screen.background());
				cursor_state = false;
			} else {
				M5.Display.fillRect(cursor_x, cursor_y, cursor_width, cursor_height, color);
				cursor_state = true;
			}
		}
...
void M5ez::_wrapLines(String text, uint16_t width, std::vector<line_t>& lines) {
	...
	while (!all_done) {
		...
		if (M5.Display.textWidth(text.substring(offset, cur_space)) > width || text.substring(last_space, last_space + 1) == (String)char(nlchar)) {
			if (M5.Display.textWidth(text.substring(offset, last_space)) <= width) {
				...
				for (uint16_t n = offset; n < text.length(); n++) {
					if (M5.Display.textWidth(text.substring(offset, n + 1)) > width) {
						new_line.position = offset;
...
int16_t M5ez::charsFit(String input, int16_t cutoff) {
	...
	for (int16_t n = input.length(); n >= 0; n--) {
		measured = M5.Display.textWidth(input.substring(0, n));
		if (measured <= cutoff) {
			return n;
		}
	}
...
String M5ez::clipString(String input, int16_t cutoff, bool dots /* = true */ ) {
	if (M5.Display.textWidth(input) <= cutoff) {
		return input;
	} else {
		for (int16_t n = input.length(); n >= 0; n--) {
			String toMeasure = input.substring(0, n);
			if (dots) toMeasure = toMeasure + "..";
			if (M5.Display.textWidth(toMeasure) <= cutoff) return toMeasure;
		}
		return "";
	}
}
...
void M5ez::setFont(const GFXfont* font) {
	...
	if (ptrAsInt <= 16) {
		...
		M5.Display.setTextFont(ptrAsInt);
	} else {
...
int16_t M5ez::fontHeight() { return M5.Display.fontHeight(); }
...
void ezMenu::_drawItem(int16_t n, String text, bool selected) {
	...
	if (selected) {
		...
		M5.Display.setTextColor(ez.theme->menu_sel_fgcolor);
	} else {
		fill_color = ez.screen.background();
		M5.Display.setTextColor(ez.theme->menu_item_color);
	}
	
	text = ez.clipString(text, TFT_W - ez.theme->menu_lmargin - 2 * ez.theme->menu_item_hmargin - ez.theme->menu_rmargin);
	M5.Display.fillRoundRect(ez.theme->menu_lmargin, top_item_h + n * _per_item_h - 2, TFT_W - ez.theme->menu_lmargin - ez.theme->menu_rmargin, _per_item_h, ez.theme->menu_item_radius, fill_color);
	M5.Display.drawString(ez.leftOf(text, "\t"), ez.theme->menu_lmargin + ez.theme->menu_item_hmargin, top_item_h + _per_item_h / 2 + n * _per_item_h );
	
	if (text.indexOf("\t") != -1) {
		M5.Display.setTextDatum(CR_DATUM);
		M5.Display.drawString(ez.rightOf(text, "\t"),  TFT_W - ez.theme->menu_rmargin - ez.theme->menu_item_hmargin, top_item_h + _per_item_h / 2 + n * _per_item_h - 2);
	}
}
...
void ezMenu::_drawImage(MenuItem_t &item) {
	if (item.image) {
		M5.Display.drawJpg((uint8_t *)item.image, (sizeof(item.image) / sizeof(item.image[0])), 0, ez.canvas.top() + _img_from_top, TFT_W, ez.canvas.height() - _img_from_top);
	}
	if (item.fs) {
		M5.Display.drawJpgFile(*(item.fs), item.path.c_str(), 0, ez.canvas.top() + _img_from_top, TFT_W, ez.canvas.height() - _img_from_top);
	}
}
...
void ezMenu::_drawCaption() {
	...
	M5.Display.setTextColor(_img_caption_color);
	M5.Display.setTextDatum(_img_caption_location);
	...
	M5.Display.drawString(caption, x, y);
}
...
void ezMenu::_Arrows() {
	...
	M5.Display.fillTriangle(l, top + 25, l + 10, top + 25, l + 5, top + 10, fill_color);
	...
	M5.Display.fillTriangle(l, top + height - 25, l + 10, top + height - 25, l + 5, top + height - 10, fill_color);		
}
...
ezProgressBar::ezProgressBar(String header /* = "" */, String msg /* = "" */, String buttons /* = "" */, const GFXfont* font /* = NULL */, uint16_t color /* = NO_COLOR */, uint16_t bar_color /* = NO_COLOR */, bool show_val /* = false */, uint16_t val_color /* = NO_COLOR */) {
	...
	for (uint8_t n = 0; n < ez.theme->progressbar_line_width; n++) {
		M5.Display.drawRect(ez.canvas.left() + ez.theme->msg_hmargin + n, _bar_y + n, ez.canvas.width() - 2 * ez.theme->msg_hmargin - 2 * n, ez.theme->progressbar_width - 2 * n, bar_color);
	}
}
...
void ezProgressBar::value(float val) {
	...
	M5.Display.fillRect(left, _bar_y + ez.theme->progressbar_line_width, width * val / 100, ez.theme->progressbar_width - 2 * ez.theme->progressbar_line_width, _bar_color);
	M5.Display.fillRect(left + (width * val / 100), _bar_y + ez.theme->progressbar_line_width, width - (width * val / 100), ez.theme->progressbar_width - 2 * ez.theme->progressbar_line_width, ez.screen.background());

	if (_show_val == true) {
		M5.Display.setTextDatum(CC_DATUM);
		M5.Display.setTextColor(_val_color);
		ez.setFont(ez.theme->msg_font);
		M5.Display.drawFloat(val, 0, TFT_W / 2, _bar_y + ez.theme->progressbar_width / 2 - 1);
	}
}
